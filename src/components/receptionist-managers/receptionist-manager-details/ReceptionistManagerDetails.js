...
import { MEDIA_URL, postGetReceptionistManagerById, postUpSertReceptionistManager, postRegister } from '../../../core/api/Api';
...
const ReceptionistManagerDetails = () => {
    let { id } = useParams()
    ...
    const getReceptionistManagerById = () => {
        postGetReceptionistManagerById({ id }).then(res => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            if (isNaN(id)) {
                postRegister(form_data).then(res => {
                    ...
                    if (response) {
                        if (isNaN(id)) {
                            navigate('/receptionist-manager-details/' + response?.id)
                        } else {
                ...
                postUpSertReceptionistManager(form_data).then(res => {
                    setLoading(false)
                    const response = res?.data?.success
                    if (response) {
                        setData({
                            ...d,
                            img: response?.img,
                            old_receptionist_ids: d.receptionist_ids
                        })
                        setSuccess(true)
                    } else {
                        setSuccess(false)
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل المريض" : "Receptionist Details"}
                </Typography>
                        ...
                        <Card component="form" noValidate onSubmit={onSubmit}>
                            ...
                            <CardContent>
                                <Grid container spacing={3}>
                                    ...
                                    {data?.lookups && <>
                                        <LookupSelectView onInputSelectChange={onInputSelectChange} data={data} d='receptionists' foreignKey='receptionist_ids' l={I18nManager.isRTL() ? "موظفو الاستقبال" : "Receptionists"} multiple={true} />
...
export default ReceptionistManagerDetails