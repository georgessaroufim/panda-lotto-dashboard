...
import { MEDIA_URL, postGetReceptionistById, postUpSertReceptionist, postRegister } from '../../../core/api/Api';
...
const ReceptionistDetails = () => {
    ...
    const getReceptionistById = () => {
        postGetReceptionistById({ id }).then(res => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            if (isNaN(id)) {
                postRegister(form_data).then(res => {
                    ...
                    if (response) {
                        if (isNaN(id)) {
                            navigate('/receptionist-details/' + response?.id)
                        } else {
                ...
                postUpSertReceptionist(form_data).then(res => {
                    setLoading(false)
                    const response = res?.data?.success
                    if (response) {
                        setData({ ...d, img: response?.img })
