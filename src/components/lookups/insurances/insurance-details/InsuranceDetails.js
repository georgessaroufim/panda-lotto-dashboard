...
import { postGetInsuranceById, postUpSertInsurance } from '../../../../core/api/Api';
...
const InsuranceDetails = () => {
    ...
    const getInsuranceById = () => {
        postGetInsuranceById({ id }).then(res => {
    ...
    const onSubmit = (e) => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            postUpSertInsurance(data).then(res => {
                ...
                if (response) {
                    if (isNaN(id)) {
                        navigate('/insurance-details/' + response?.id)
                    }
                    setSuccess(true)
                } else {
                    setSuccess(false)
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل التأمين" : "Insurance Details"}
                </Typography>
