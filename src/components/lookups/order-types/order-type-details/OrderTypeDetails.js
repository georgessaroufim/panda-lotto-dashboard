...
import { postGetOrderTypeById, postUpSertOrderType } from '../../../../core/api/Api';
...
const OrderTypeDetails = () => {
    ...
    const getOrderTypeById = () => {
        postGetOrderTypeById({ id }).then(res => {
    ...
    const onSubmit = (e) => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            postUpSertOrderType(data).then(res => {
                ...
                if (response) {
                    if (isNaN(id)) {
                        navigate('/order-status-details/' + response?.id)
                    }
                    setSuccess(true)
                } else {
                    setSuccess(false)
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل نوع الطلب" : "Order Type Details"}
                </Typography>
