...
import { postGetOrderStatusById, postUpSertOrderStatus } from '../../../../core/api/Api';
...
const OrderStatusDetails = () => {
    ...
    const getOrderStatusById = () => {
        postGetOrderStatusById({ id }).then(res => {
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل حالة الطلب" : "Order Status Details"}
                </Typography>
