...
import { postGetGeneralServiceById, postUpSertGeneralService } from '../../../../core/api/Api';
import { Box, Container, Button, Grid, Typography, Card, CardContent, CardHeader, Divider, TextField } from '@mui/material';
...
const GeneralServiceDetails = () => {
    ...
    const getGeneralServiceById = () => {
        postGetGeneralServiceById({ id }).then(res => {
    ...
    const onSubmit = (e) => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            postUpSertGeneralService(data).then(res => {
                ...
                if (response) {
                    if (isNaN(id)) {
                        navigate('/general-service-details/' + response?.id)
                    }
                    setSuccess(true)
    ...
    return (
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Grid sx={{ mb: 3 }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Typography sx={{ mb: 3 }} variant="h4">
                        {I18nManager.isRTL() ? "تفاصيل الخدمة العامة" : "General Service Details"}
                    </Typography>
                    <Button onClick={onCreateButtonClick} variant="contained">{I18nManager.isRTL() ? "رؤية" : "View"}</Button>
                </Grid>
...
export default GeneralServiceDetails