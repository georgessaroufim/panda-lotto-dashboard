...
import { MEDIA_URL, postGetNationalityById, postUpSertNationality } from '../../../../core/api/Api';
import { Box, Container, Button, CardActions, Grid, Avatar, Typography, Card, CardContent, CardHeader, Divider, TextField } from '@mui/material';
...
const NationalityDetails = () => {
    ...
    const getNationalityById = () => {
        postGetNationalityById({ id }).then(res => {
    ...
    const onSubmit = (e) => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            postUpSertNationality(form_data).then(res => {
                setLoading(false)
                const response = res?.data?.success
                if (response) {
                    if (isNaN(id)) {
                        navigate('/nationality-details/' + response?.id)
                    } else {
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل الجنسية" : "Nationality Details"}
                </Typography>
