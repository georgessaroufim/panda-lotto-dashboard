...
import { MEDIA_URL, postGetComplexClinicById, postUpSertComplexClinic, postRegister } from '../../../core/api/Api';
...
const ComplexClinicDetails = () => {
    ...
    const getComplexClinicById = () => {
        postGetComplexClinicById({ id }).then(res => {
        ...
        if (checkFormErrors(formErrors)) {
            ...
            if (isNaN(id)) {
                postRegister(form_data).then(res => {
                    ...
                    if (response) {
                        if (isNaN(id)) {
                            navigate('/complex-clinic-details/' + response?.id)
                        } else {
                ...
                postUpSertComplexClinic(form_data).then(res => {
                    setLoading(false)
                    const response = res?.data?.success
                    if (response) {
                        setSuccess(true)
        ...
        <Box component="main" sx={{ flexGrow: 1, }}>
            {(data?.id || isNaN(id)) && <Container maxWidth="lg">
                <Typography sx={{ mb: 3 }} variant="h4">
                    {I18nManager.isRTL() ? "تفاصيل مجمع العيادات" : "Complex Clinic Details"}
                </Typography>
...
export default ComplexClinicDetails